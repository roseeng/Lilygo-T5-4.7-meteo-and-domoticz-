...
#include "Secrets/owm_credentials.h"
...
void DisplayMQTT(int x, int y)
{
  ...
  for (int j = 0; j <= 3; j++)
  {
    ...
    for (int i = j * 3; i <= (j * 3 + 2); i++)
    {
      ...
      { // prevent hang null value
        int ycorrect = 0;
        if (domoticz_IDX[i][0].indexOf("y") != -1 || domoticz_IDX[i][0].indexOf("p") != -1)
          ycorrect = -6; /// correction du big polices  ( 12b)
  ...
  if (domoticz_result[0].substring(0, 2).toInt() > wash)
  {
    DrawWashingImage(x + 500, y + 35);
  } // si temp eau > 45 > ok pour machine laver
  if (domoticz_result[4].substring(0, 4).toInt() > disher)
    DrawDisherImage(x + 575, y + 35); // si prod > 700w alors ok pour lave vaisselle
}
